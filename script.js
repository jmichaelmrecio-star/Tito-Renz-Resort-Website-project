...
function renderAmenitiesGrid() {
    ...
    // Get filter values
    const typeFilter = document.getElementById('amenity-type-filter')?.value || 'all';
    const capFilter = parseInt(document.getElementById('amenity-capacity-filter')?.value || '0', 10);
    ...
    if (typeFilter !== 'all') {
        filtered = filtered.filter(s => {
            ...
            console.log(`Service: ${s.name}, Type: ${type}, Filter: ${typeFilter}, Match: ${type === typeFilter}`);
    ...
    grid.innerHTML = filtered.map(service => {
        ...
        <div class="amenity-card">
            ...
            <div class="card-body">
                ...
                <div class="amenity-meta">Starts at ₱${startingPrice.toFixed(2)} &bull; Max Guests: ${service.max_guests}</div>
                <div class="amenity-actions">
                    <button class="button-secondary" onclick="openAmenityGallery('${service.id}')">More Photos</button>
                    <a class="button-primary" href="services-list.html?serviceId=${service.id}">Book Now</a>
                </div>
            </div>
        </div>
    `;
...
function openAmenityGallery(serviceId) {
    ...
    const images = service.gallery && Array.isArray(service.gallery) && service.gallery.length > 0
        ? service.gallery
        : [service.image];

    // Show images larger for better examination (no inline styles, use CSS)
    gallery.innerHTML = images.map(img => `<img src="${img}" alt="${service.name}">`).join('');
...
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('amenities-grid')) {
        renderAmenitiesGrid();
        document.getElementById('amenity-type-filter').addEventListener('change', renderAmenitiesGrid);
        document.getElementById('amenity-capacity-filter').addEventListener('change', renderAmenitiesGrid);
        document.getElementById('close-amenity-gallery').addEventListener('click', closeAmenityGallery);
        document.getElementById('amenity-gallery-modal').addEventListener('click', function(e) {
...
function renderAmenities(services) {
    ...
    services.forEach(service => {
        ...
        card.innerHTML = `
            <img src="${service.image}" alt="${service.name}" style="width:100%;height:auto;border-radius:8px;object-fit:cover;">
            <div class="card-body">
                <h3>${service.name}</h3>
                <div class="amenity-desc">${service.description}</div>
                <div class="amenity-meta">Starts at ₱${startingPrice.toFixed(2)} &bull; Max Guests: ${service.max_guests}</div>
...
function handleFilterChange() {
    ...
    if (selectedType && selectedType !== 'all') {
        filteredServices = filteredServices.filter(service => service.type === selectedType);
    }
    
    // Filter by capacity (show services that can accommodate UP TO the selected number)
    if (selectedCapacity > 0) {
        filteredServices = filteredServices.filter(service => service.max_guests >= selectedCapacity);
    }
...
function getReservationKey(reservation = {}) {
    return reservation._id 
        ...
        || `${(reservation.email || 'guest').toLowerCase()}-${reservation.check_in || reservation.checkin_date || reservation.createdAt || Date.now()}`;
}
...
function getLocalReservationsByEmail(email = '') {
    ...
    return (reservations || []).filter(res => (res.email || '').toLowerCase() === normalizedEmail);
}
...
function getReservationStatus(reservation = {}) {
    return normalizeStatusValue(reservation.status || reservation.paymentStatus || reservation.statusText);
}
...
function storeReservationLocally(serverResult = {}, reservationData = {}) {
    ...
    const safeTotal = normalizeCurrencyValue(reservationData.finalTotal || reservationData.finalTotalInput);
    const statusValue = normalizeStatusValue(serverResult.status || serverResult.paymentStatus || reservationData.status);
    const normalizedRecord = {
        _id: generateReservationId(serverResult.reservationId || serverResult._id),
        serviceType: reservationData.serviceType || reservationData.serviceTypeInput || 'Unknown Service',
        check_in: reservationData.checkin_date || reservationData.check_in || new Date().toISOString(),
        finalTotal: safeTotal,
        status: statusValue,
        paymentStatus: statusValue,
        qrCodeHash: serverResult.reservationHash || serverResult.qrCodeHash || null,
        full_name: reservationData.customer_name || loggedUser?.full_name || 'Guest User',
        email: reservationData.customer_email || loggedUser?.email || '',
        gcashReferenceNumber: serverResult.gcashReferenceNumber || reservationData.gcashReferenceNumber || '',
...
function renderLocalUserReservations(listElement, userEmail) {
    ...
    const localMatches = (reservations || []).filter(res => (res.email || '').toLowerCase() === email);

    if (localMatches.length === 0) {
        listElement.innerHTML = '<tr><td colspan="6" style="text-align: center;">You have no reservations on record.</td></tr>';
        return false;
    }
...
function renderLocalAdminReservations(listElement) {
    ...
    if (localData.length === 0) {
        listElement.innerHTML = '<tr><td colspan="9" style="text-align: center;">No reservations recorded yet.</td></tr>';
        return false;
    }
...
async function registerUser(event) {
    ...
    const middleName = document.getElementById('registerMiddleInitial').value.trim();
    ...
    try {
        ...
        if (response.ok) {
            ...
            showModal('Registration Failed', `<p>${data.message || 'An unknown error occurred.'}</p>`, 'error');
        }
 
    } catch (error) {
        console.error('Network or client-side error during registration:', error);
        showModal('Connection Error', '<p>Could not connect to the server. Please check your network and try again.</p>', 'error');
    }
}
...
async function loginUser(event) {
    ...
    if (!email || !password) {
        showModal('Missing Information', '<p>Please enter both email and password to continue.</p>', 'warning');
        return;
    }
    ...
    try {
        ...
        showToast(`Login successful! Redirecting to the ${data.user.role === 'Customer' ? 'home page' : 'Admin Dashboard'}.`, 'success');
        setTimeout(() => {
        ...
        showModal('Connection Error', '<p>An error occurred during login. Please check the server connection and try again.</p>', 'error');
    }
}
...
function checkAuthAndRedirect(requireAdmin = false) {
    ...
    const requiresAuth = window.location.pathname.includes('profile.html') || 
                         window.location.pathname.includes('admin-dashboard.html');
 
    if (!requiresAuth) return;
 
    if (!user) {
        showModal('Authentication Required', '<p>You must be logged in to access this page. Redirecting to login...</p>', 'warning');
        setTimeout(() => {
            window.location.href = 'login.html';
        }, 1500);
        return;
    ...
    if (requireAdmin && user.role !== 'Admin' && user.role !== 'Manager') {
        showModal('Access Denied', '<p>You do not have admin privileges to access this page. Redirecting...</p>', 'error');
        setTimeout(() => {
...
function checkLoginStatusForReservation() {
    ...
    if (user) {
        ...
        if (nameEl) {
            nameEl.value = user.full_name || `${user.first_name || ''} ${user.last_name || ''}`.trim();
        ...
        if (!info) {
            ...
            info.textContent = 'Your account details are pre-filled. You may update your contact number or address.';
...
async function renderUserReservations() {
    ...
    list.innerHTML = '<tr><td colspan="6" style="text-align: center;">Loading reservation history...</td></tr>';
    ...
    try {
        ...
        const combinedReservations = mergeReservationLists(userReservations, localMatches);

        if (combinedReservations.length === 0) {
            list.innerHTML = '<tr><td colspan="6" style="text-align: center;">You have no reservations on record.</td></tr>';
            return;
        }
        ...
        if (!rendered) {
            list.innerHTML = '<tr><td colspan="6" style="color: red; text-align: center;">Error loading reservations. Please check your Node.js API console.</td></tr>';
        }
    }
}
...
function openProfileEditModal() {
    const user = getLoggedInUser();
    if (!user) {
        showModal('Login Required', '<p>Please log in to edit your profile.</p>', 'warning', {
...
async function handleProfileEditSubmit(event) {
    ...
    if (!user || !token) {
        showModal('Login Required', '<p>Please log in again to update your profile.</p>', 'warning', {
            onClose: () => window.location.href = 'login.html'
        });
        return;
    }
    ...
    if (!firstName || !lastName || !email || !phone) {
        showToast('Please fill out all required fields (First Name, Last Name, Email, Contact).', 'warning');
        return;
    }
...
function openEditAccountModalById(userId) {
    ...
    if (!user) {
        showModal('User Not Found', '<p>The requested user could not be found. Please refresh the page and try again.</p>', 'error');
        return;
    }
...
function openEditAccountModal(userId, user) {
    ...
    document.getElementById('passwordLabel').textContent = '(Leave blank to keep current password):';
...
async function handleAccountFormSubmit(event) {
    ...
    const selectedRole = allRoles.find(r => r.role_name.toLowerCase() === roleValue);
    if (!selectedRole) {
    ...
    showConfirm(
        ...
        async () => {
            try {
                ...
                if (!response.ok) {
                    showModal('Deactivation Failed', `<p>${data.message || 'Failed to deactivate account.'}</p>`, 'error');
                    return;
                }
                ...
                console.error('Error:', error);
                showModal('Network Error', '<p>A network error occurred. Please try again.</p>', 'error');
            }
        },
    ...
    showConfirm(
        ...
        async () => {
            try {
                ...
                if (!response.ok) {
                    showModal('Activation Failed', `<p>${data.message || 'Failed to activate account.'}</p>`, 'error');
                    return;
                }
...
async function renderUsersList() {
    ...
    userTableBody.innerHTML = '<tr><td colspan="7" style="text-align: center;">Fetching users from API...</td></tr>';
    ...
    try {
        ...
        users.forEach(user => {
            ...
            actionsCell.innerHTML = `
                <button class="button-small button-secondary" onclick="openEditAccountModalById('${user._id}')">✏️ Edit</button>
                ${user.isActive 
...
async function renderAdminReservations() {
    ...
    if (userRole !== 'admin' && userRole !== 'manager') {
        if (list) {
            list.innerHTML = '<tr><td colspan="9" style="text-align: center; color: red;">ACCESS DENIED: Insufficient permissions.</td></tr>';
        }
        return;
    }
    ...
    list.innerHTML = '<tr><td colspan="9" style="text-align: center;">Loading pending reservations...</td></tr>';
    ...
    try {
        ...
        const localData = Array.isArray(reservations) ? reservations : [];
        const combinedReservations = mergeReservationLists(apiReservations, localData);

        if (combinedReservations.length === 0) {
            list.innerHTML = '<tr><td colspan="9" style="text-align: center;">No new paid reservations currently pending review.</td></tr>';
            return;
        }
        ...
        if (!rendered && list) {
            list.innerHTML = '<tr><td colspan="9" style="color: red; text-align: center;">Error: Could not connect to Admin API.</td></tr>';
        }
    }
}
// --- Admin Actions ---
    ...
    showConfirm(
        ...
        async () => {
            try {
                ...
                if (!response.ok) {
                    showModal('Confirmation Failed', `<p>${data.message || 'Server error during confirmation.'}</p>`, 'error');
                    console.error('Confirmation server error:', data);
                    return;
                }
         
                showToast('Reservation marked as CONFIRMED. Ready for check-in.', 'success');
        
                updateLocalReservation(reservationId, { status: 'CONFIRMED', paymentStatus: 'PAID' });
                ...
                showModal('Network Error', '<p>A network error occurred. Could not confirm reservation. Please try again.</p>', 'error');
            }
        },
    ...
    showConfirm(
        ...
        async () => {
            try {
                ...
                if (!response.ok) {
                    showModal('Payment Mark Failed', `<p>${data.message || 'Server error.'}</p>`, 'error');
                    console.error('Server error:', data);
                    return;
                }
        
                showToast('Reservation marked as PAID.', 'success');
                updateLocalReservation(reservationId, { status: 'paid', paymentStatus: 'paid' });
                
                if (typeof renderAdminReservations === 'function') renderAdminReservations();
        
            } catch (error) {
                console.error('Network error:', error);
                showModal('Network Error', '<p>A network error occurred. Could not mark as paid. Please try again.</p>', 'error');
            }
        },
    ...
    showConfirm(
        ...
        async () => {
            try {
                ...
                if (!response.ok) {
                    showModal('Cancellation Failed', `<p>${data.message || 'Server error during cancellation.'}</p>`, 'error');
                    console.error('Cancellation server error:', data);
                    return;
                }
         
                showToast('Reservation cancelled successfully! It will be removed from the pending list shortly.', 'success');
        
                updateLocalReservation(reservationId, { status: 'cancelled', paymentStatus: 'cancelled' });
                
                // Refresh the list to remove the cancelled item immediately
                if (typeof renderAdminReservations === 'function') renderAdminReservations();
            } catch (error) {
                console.error('Network error during cancellation:', error);
                showModal('Network Error', '<p>A network error occurred. Could not cancel reservation. Please try again.</p>', 'error');
            }
        },
...
function toggleGuestDetailsForm() {
    ...
    if (guestForm) {
        if (window.isLoggedIn) {
            ...
            guestForm.querySelectorAll('input, select').forEach(el => el.removeAttribute('required'));
        } else {
            // Guest user: Show the form and ensure 'required' attributes are set
            guestForm.style.display = 'block';
            guestForm.querySelectorAll('input, select').forEach(el => el.setAttribute('required', ''));
        }
    }
}
...
async function loadServicesForBlocking() {
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to load services: ${response.status} ${response.statusText} - ${txt.slice(0,200)}`);
        }
        ...
        (services || []).forEach(service => {
            ...
            const checkboxValue = service.id || service._id || service.idString || service.name;
            const displayName = service.name || service.id || service._id || 'Unknown Service';
        ...
        showModal('Error', '<p>Failed to load services. Check server console and ensure /api/services is available.</p>', 'error');
    }
}
...
function setupBlockDateForm() {
    blockDateForm.addEventListener('submit', async (e) => {
        ...
        if (startDateObj < today) {
            showModal('Invalid Date', '<p>Cannot block dates in the past. Please select today or a future date.</p>', 'warning');
            return;
        }
        
        // Get selected service IDs (frontend IDs)
        const selectedServiceIds = Array.from(serviceCheckboxes.querySelectorAll('input[type="checkbox"]:checked'))
        ...
        try {
            ...
            blockDateForm.reset();
            loadBlockedDates();
            showModal('Success', '<p>Date range blocked successfully!</p>', 'success');
            
        } catch (error) {
            console.error('Error blocking date range:', error);
            showModal('Error', `<p>${error.message || 'Failed to block date range. Please try again.'}</p>`, 'error');
        }
    });
}
...
function renderBlockedDates(blockedDates) {
    ...
    if (!Array.isArray(blockedDates) || blockedDates.length === 0) {
        blockedDatesList.innerHTML = '<p style="color: #999; padding: 10px;">No blocked dates found.</p>';
        return;
    }
            ...
            <table class="reservation-table" style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    ${blockedDates.map(block => {
                        const start = block.startDate || block.start || block.date;
                        const end = block.endDate || block.end;
                        const reason = block.reason || block.reasonText || 'No reason provided';
                        const appliesAll = block.appliesToAll || block.appliesToAllServices || (!block.serviceIds || block.serviceIds.length === 0);
                        const id = block._id || block.id || '';
                        
                        // Get service names from IDs
                        let appliesTo = 'All Services';
                        if (!appliesAll && block.serviceIds && block.serviceIds.length > 0) {
                            const allServices = getActiveServices();
                            const serviceNames = block.serviceIds.map(serviceId => {
                                // Try to find by MongoDB _id or frontend id
                                const service = allServices.find(s => s._id === serviceId || s.id === serviceId);
                        ...
                        return `
                        <tr>
                            <td style="padding: 12px; border: 1px solid #dee2e6;">${safeFormatDate(start)} to ${safeFormatDate(end)}</td>
                            <td style="padding: 12px; border: 1px solid #dee2e6;">${escapeHtml(reason)}</td>
    ...
    showConfirm(
        ...
        async () => {
            try {
                ...
                showModal('Error', error.message || 'Failed to unblock date range. Please try again.', 'error');
            }
        },
        {
            confirmText: 'OK',
...
function updateBookingSummaryDisplay(service) {
    ...
    if (inclusionsWrap && inclusionsList) {
        if (service.inclusions && service.inclusions.length > 0) {
            inclusionsList.innerHTML = service.inclusions.map(item => `<li>${item}</li>`).join('');
...
function showServiceModal(serviceId) {
    ...
    if (durationSelect) {
        durationSelect.innerHTML = '<option value="">-- Select Duration/Time --</option>';
        ...
        if (service.timeSlots && service.timeSlots.length > 0) {
            ...
            if (service.timeSlots.length > 0) {
                ...
                // Show inclusions
                if (service.inclusions && service.inclusions.length > 0) {
                    inclusionsList.innerHTML = service.inclusions.map(inc => `<li>${inc}</li>`).join('');
        ...
        // Add durations if available (for rooms and halls)
            ...
            service.durations.forEach(duration => {
                ...
                option.textContent = `${duration.label} - ₱${duration.price.toFixed(2)}`;
                durationSelect.appendChild(option);
            });
            ...
            if (service.defaultDuration) {
                ...
                const defaultDur = service.durations.find(d => d.id === service.defaultDuration);
                if (defaultDur) {
        ...
        durationSelect.addEventListener('change', function() {
            ...
            if (service.timeSlots && service.timeSlots.length > 0) {
                const slot = service.timeSlots.find(ts => ts.id === selectedDurationId);
                if (slot) {
                    selectedPrice = slot.price;
                }
            } else if (service.durations && service.durations.length > 0) {
                const duration = service.durations.find(d => d.id === selectedDurationId);
                if (duration) {
    ...
    if (!(service.timeSlots && service.timeSlots.length > 0) && 
        !(service.durations && service.durations.length > 0 && service.defaultDuration)) {
        priceDisplay.textContent = 'Select duration';
        proceedButton.style.pointerEvents = 'none';
        proceedButton.style.opacity = '0.5';
    }
    ...
    proceedButton.onclick = () => {
        ...
        if (!selectedDurationId) {
            showModal('Missing Selection', '<p>Please select a duration or time slot before proceeding.</p>', 'warning');
            return false;
        }
        selectServiceAndRedirect(service, selectedDurationId);
        return false;
...
function selectServiceAndRedirect(service, durationId) {
    ...
    if (paymentSubmissionInProgress) {
        ...
        console.warn('Service re-selection prevented: payment submission in progress');
        return;
    }
    ...
    const serviceData = {
        ...
        serviceInclusions: service.inclusions ? JSON.stringify(service.inclusions) : '[]'
    };
...
function renderServiceSelectionSummary() {
    ...
    if (durationSelector && serviceId) {
        ...
        if (service) {
            ...
            durationSelector.innerHTML = `<option value="${selectedDuration}" data-price="${fallbackPrice}">${selectedDurationLabel || 'Selected Option'} - ₱${fallbackPrice}</option>`;
        }
        ...
        durationSelector.addEventListener('change', function() {
            ...
            if (service && service.timeSlots && service.timeSlots.find(ts => ts.id === durationId)) {
    ...
    if (selectedDurationInput && selectedDuration) {
        ...
        if (service && service.timeSlots && service.timeSlots.find(ts => ts.id === selectedDuration)) {
...
async function setupDateValidation() {
    ...
    async function validateDates() {
        ...
        try {
            ...
            const blockedDates = Array.isArray(blockedData) ? blockedData : (blockedData.blockedDates || []);
...
async function highlightUnavailableDates() {
    const checkinInput = document.getElementById('checkin');
    const availabilitySection = document.getElementById('availabilityCalendarSection');
    ...
    try {
        ...
        if (unavailableList) {
            ...
            const relevantBlocks = blockedDates.filter(block => {
                ...
                console.log('Block:', block, 'ServiceIds:', block.serviceIds, 'Matches MongoDB ID?', isRelevantById, 'Matches Frontend ID?', isRelevantByFrontendId, 'Is relevant?', isRelevant);
                return isRelevant;
            });
            
            console.log('Filtered relevant blocks for service', serviceId, '(MongoDB:', mongoId, '):', relevantBlocks);
            
            // Add blocked dates
            if (relevantBlocks.length > 0) {
                unavailableHTML += '<div style="margin-bottom: 15px;"><strong style="color: #dc3545;">\ud83d\udeab Blocked Dates:</strong><ul style="margin: 5px 0; padding-left: 20px;">';
                relevantBlocks.forEach(block => {
                    const start = safeFormatDate(block.startDate || block.start || block.date);
                    const end = safeFormatDate(block.endDate || block.end);
                    unavailableHTML += `<li style="margin: 5px 0;">${start} to ${end} - ${block.reason || 'Unavailable'}</li>`;
                });
            ...
            const confirmedReservations = reservations.filter(r => {
                ...
                const hasValidDates = (r.check_in || r.checkin_date) && (r.check_out || r.checkout_date);
                return r.status !== 'CANCELLED' && hasValidDates;
            });
            if (confirmedReservations.length > 0) {
                unavailableHTML += '<div><strong style="color: #ffc107;">\ud83d\udcc5 Reserved Dates:</strong><ul style="margin: 5px 0; padding-left: 20px;">';
            ...
            if (!unavailableHTML) {
                unavailableHTML = '<p style="color: #28a745; font-weight: bold;">\u2705 All dates are currently available for this service!</p>';
            }
        ...
        reservations.forEach(res => {
            ...
            const isNotCancelled = res.status !== 'cancelled' && res.status !== 'rejected';
...
async function editService(serviceId) {
    try {
        ...
        if (imageNameDisplay) imageNameDisplay.textContent = service.image || 'None';
        if (galleryCountDisplay) galleryCountDisplay.textContent = service.gallery ? service.gallery.length : 0;
...
async function createPromoCode(event) {
    ...
    if (!code || isNaN(discountPercentage) || !expirationDate) {
        showModal('Missing Information', '<p>Please fill in all required fields:</p><ul><li>Promo Code</li><li>Discount Percentage</li><li>Expiration Date</li></ul>', 'warning');
        return;
    }
    ...
    try {
        ...
        codes.forEach(code => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <button class="button-small button-danger" onclick="deletePromoCode('${code._id}')">
                        Delete
                    </button>
                </td>
            `;
...
async function deletePromoCode(codeId) {
    showConfirm(
        ...
        async () => {
            try {
                const response = await fetch(`http://localhost:3000/api/promocodes/${codeId}`, {
                    method: 'DELETE'
                });
...
async function renderPublicPromotions() {
    ...
    try {
        ...
        const active = (codes || []).filter(code => {
            const exp = new Date(code.expirationDate || code.expires || code.expireAt);
            const notExpired = !isNaN(exp.getTime()) ? exp >= today : true;
            const underLimit = typeof code.usageLimit === 'number' ? (code.timesUsed || 0) < code.usageLimit : true;
        ...
        active.forEach(promo => {
            const pct = (promo.discountPercentage != null) ? (promo.discountPercentage * 100).toFixed(0) : (promo.discount || '');
            const exp = new Date(promo.expirationDate || promo.expires || promo.expireAt).toLocaleDateString();
            html += `
...
async function renderPublicBlockedDates() {
    ...
    try {
        ...
        let response = await fetch('http://localhost:3000/api/blocked-dates/active/list');
        if (!response.ok) {
        ...
        if (!Array.isArray(list) || list.length === 0) {
            container.innerHTML = '<p class="no-blocks">No blocked dates at the moment.</p>';
            return;
        }

        let html = '<div class="blocked-list">';
        list.forEach(block => {
            const applies = block.appliesToAll ? 'All Services' : (block.serviceIds && block.serviceIds.length ? block.serviceIds.join(', ') : 'Selected Services');
            html += `
...
async function fetchAllServicesAdmin() {
    try {
        ...
        const services = await response.json();
        console.log(`✅ Admin fetched ${services.length} services (including inactive)`);
        return services;
    } catch (error) {
...
function getActiveServices() {
    ...
    if (window.cachedServices && Array.isArray(window.cachedServices)) {
        const activeServices = window.cachedServices.filter(s => s.isActive !== false);
...
async function deactivateService(serviceId) {
    const token = localStorage.getItem('token');
    if (!token) {
        showModal('Authentication Required', '<p>You must be logged in as an admin to deactivate services.</p>', 'warning');
        return null;
    }
...
async function renderServiceTable() {
    ...
    try {
        ...
        if (services.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="7" class="text-center">No services found.</td></tr>';
            return;
        }

        tableBody.innerHTML = '';
        services.forEach(service => {
            const statusBadge = service.isActive !== false ? 
                '<span style="color: green; font-weight: bold;">✓ Active</span>' :
                '<span style="color: gray; font-weight: bold;">✗ Inactive</span>';
            
            const actionBtn = service.isActive !== false ?
        ...
        tableBody.innerHTML = '<tr><td colspan="7" class="text-center text-danger">Error loading services.</td></tr>';
    }
}
...
async function activateServiceAndRefresh(serviceId) {
    showConfirm(
        ...
        async () => {
            const token = localStorage.getItem('token');
            if (!token) {
                showModal('Authentication Required', '<p>You must be logged in as an admin to activate services.</p>', 'warning');
                return;
            }
...
async function handleCreateServiceForm(event) {
    ...
    const max_guests = parseInt(document.getElementById('serviceMaxGuestsInput').value, 10);
    ...
    if (!serviceImageInput.files || serviceImageInput.files.length === 0) {
        showModal('Missing Image', '<p>Please select a main image for this service.</p>', 'warning');
        return;
    }
    ...
    let gallery = [];
    if (serviceGalleryInput && serviceGalleryInput.files && serviceGalleryInput.files.length > 0) {
        gallery = Array.from(serviceGalleryInput.files).map(file => 'images/' + file.name);
    }
    
    const inclusionsStr = document.getElementById('serviceInclusionsInput').value.trim();
    const notes = document.getElementById('serviceNotesInput').value.trim();

    if (!id || !name || !type || !category || !max_guests || !description) {
        showModal('Missing Information', '<p>Please fill in all required fields before submitting.</p>', 'warning');
        return;
    }

    // Parse inclusions
    const inclusions = inclusionsStr ? inclusionsStr.split(',').map(i => i.trim()) : [];
    ...
    durationRows.forEach(row => {
        ...
        const durationPrice = parseFloat(row.querySelector('.duration-price').value);

        if (!durationId || !durationLabel || isNaN(durationPrice)) {
            showModal('Incomplete Fields', '<p>Please fill in all duration/timeslot fields.</p>', 'warning');
            return;
        }

        if (isTimeSlot) {
            // Time slot (for venues with guest ranges)
            const guestMin = parseInt(row.querySelector('.duration-guest-min').value, 10);
            const guestMax = parseInt(row.querySelector('.duration-guest-max').value, 10);
            const timeRange = row.querySelector('.duration-time-range').value;

            if (isNaN(guestMin) || isNaN(guestMax) || !timeRange) {
                showModal('Incomplete Timeslot', '<p>Please fill in guest range and time range for all timeslots.</p>', 'warning');
                return;
            }
            ...
            if (isNaN(hours)) {
                showModal('Incomplete Duration', '<p>Please fill in the number of hours for all durations.</p>', 'warning');
                return;
            }
    ...
    if (durations.length === 0 && timeSlots.length === 0) {
        showModal('Missing Duration', '<p>Please add at least one duration or timeslot for this service.</p>', 'warning');
        return;
    }
...
function addDurationRow() {
    ...
    row.style.cssText = 'display: flex; gap: 10px; margin-bottom: 10px; padding: 10px; background: #f9f9f9; border-radius: 4px; align-items: flex-start; flex-wrap: wrap;';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (promoCreateForm) promoCreateForm.addEventListener('submit', createPromoCode);
    ...
    if (serviceCreateForm) serviceCreateForm.addEventListener('submit', handleCreateServiceForm);
    if (addDurationBtn) addDurationBtn.addEventListener('click', (e) => {
        e.preventDefault();
        addDurationRow();
    });
    ...
    if (document.getElementById('public-blocked-dates')) renderPublicBlockedDates();

    // If on the reservation page, prefill guest details when logged in
    if (document.getElementById('guestDetailsForm')) checkLoginStatusForReservation();
...
async function reserveNow(event) {
    ...
    if (reservationSubmissionInProgress) {
        console.warn('Reservation submission already in progress, ignoring duplicate request');
        showModal('Submission In Progress', '<p>Your reservation is currently being submitted. Please wait...</p>', 'warning');
        return;
    }
    ...
    if (maxGuestsInput) {
        ...
        if (parseInt(numberOfGuests, 10) > maxGuests) {
            alert(`The selected service only allows a maximum of ${maxGuests} guests. Please adjust the number of guests.`);
        ...
        console.warn("Missing 'maxGuestsAllowed' element for client-side validation.");
    }
    ...
    if (serviceId && checkinValue && checkoutValue) {
        ...
        try {
            ...
            const reservationsResponse = await fetch(`http://localhost:3000/api/reservations/service/${serviceIdForQuery}`);
            if (reservationsResponse.ok) {
                const reservationsData = await reservationsResponse.json();
                const allReservations = Array.isArray(reservationsData) ? reservationsData : (reservationsData.reservations || []);
                ...
                for (const reservation of allReservations) {
                    ...
                    const isSameService = reservation.serviceId === serviceId || 
                    ...
                    const existingCheckin = new Date(reservation.check_in || reservation.checkin_date);
                    const existingCheckout = new Date(reservation.check_out || reservation.checkout_date);
                    ...
                    if (hasOverlap) {
                        ...
                        const existingCheckoutStr = existingCheckout.toLocaleString('en-US', {
                            year: 'numeric',
                            month: 'short',
                            day: 'numeric',
                            hour: '2-digit',
                        ...
                        showModal(
                            ...
                            <ul style="list-style: none; padding-left: 0; margin: 10px 0;">
                                ...
                                <li>👤 Customer: ${reservation.full_name || 'Reserved'}</li>
                                <li>🔖 Status: ${reservation.status || 'PENDING'}</li>
                            </ul>
    ...
    if (serviceTypeName && finalServiceId) {
        ...
        alert('Error: Required service data is missing. Please re-select a service.');
    ...
    if (!reservationData.selectedDuration && !reservationData.selectedTimeSlot) {
        alert('Error: Duration or time slot not selected. Please refresh and try again.');
        return;
    }
    // --- END FAILSAFE ---
    ...
    if (isResuming === 'true' && currentReservationId) {
        ...
        const paymentUrl = `payment.html?reservationId=${currentReservationId}&hash=${sessionStorage.getItem('current_reservation_hash')}`;
...
function recoverServiceDataFromStorage() {
    const sessionKeys = ['selectedServiceId', 'selectedServiceName', 
                         'selectedServicePrice', 'selectedServiceMaxGuests', 'selectedServiceType',
                         'selectedDuration', 'selectedDurationLabel', 'serviceInclusions',
    ...
    if (recovered) {
        console.log('✅ Service data recovered from localStorage to sessionStorage');
    }
...
function triggerServiceSelectionAutoSave(service, durationId) {
    ...
    autoSaveServiceTimeout = setTimeout(() => {
        ...
        if (typeof saveReservationProgress === 'function') {
            ...
            console.log('Service selection auto-saved during modal interaction:', { serviceId: service.id, durationId });
        }
    }, 1000);
}
...
async function processGCashPayment(event, reservationId, reservationHash) {
    ...
    if (!finalReservationHash || !finalReservationId || !gcashReferenceNumber || gcashReferenceNumber.length < 10) {
        ...
        alert('Missing critical payment data. Please ensure the GCash reference is entered and try again.');
    ...
    try {
        const response = await fetch('http://localhost:3000/api/reservations/finalize-reservation', { 
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (!response.ok) {
            alert(`Payment Failed: ${data.message || 'An error occurred during finalization.'}`);
        ...
        const clearKeys = ['selectedServiceId', 'selectedServiceName', 
                           'selectedServicePrice', 'selectedServiceMaxGuests', 'selectedServiceType',
                           'selectedDuration', 'selectedDurationLabel', 'serviceInclusions',
                           'customerName', 'finalTotal', 'current_reservation_id', 'current_reservation_hash',
                           'payment_reservation_id', 'payment_reservation_hash'];
        ...
        showModal('Payment Confirmed', '<p>✅ Your payment has been successfully processed!</p><p>Your reservation is now PAID. Redirecting to confirmation page...</p>', 'success');
        ...
        showModal('Payment Error', '<p>❌ A network error occurred while processing your payment.</p><p>Please try submitting the payment again.</p>', 'error');
...
async function fetchAndDisplaySummary(reservationId, reservationHash) {
    ...
    try {
        ...
        document.getElementById('serviceNameDisplay').textContent = reservation.serviceType || 'N/A';
        document.getElementById('summaryCustomerName').textContent = reservation.full_name || 'Guest User';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    checkAuthAndRedirect(window.location.pathname.includes('admin-dashboard.html'));

    const typeFilterElement = document.getElementById('amenity-type-filter');
    const capacityFilterElement = document.getElementById('amenity-capacity-filter');
    if (typeFilterElement) {
        // Only run this logic on amenities.html
        typeFilterElement.addEventListener('change', handleFilterChange);
        if (capacityFilterElement) capacityFilterElement.addEventListener('change', handleFilterChange);
    ...
    if (serviceListContainer) {
        ...
        if (typeFilterEl) typeFilterEl.addEventListener('change', applyServiceFilters);
        if (capFilterEl) capFilterEl.addEventListener('change', applyServiceFilters);
    }
...
if (window.location.pathname.includes('reserve.html')) {
    ...
    if (!finalServiceId) {
        showModal('No Service Selected', '<p>Please select a service before making a reservation.</p>', 'warning');
